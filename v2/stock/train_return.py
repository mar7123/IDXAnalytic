...
def build_return_model(engine: Engine, rand: int):
    config_manager.stock_profile_mapper
    ...
    X_train, X_train_id, y_train, X_train_tree, y_train_tree = make_return_sequences(
        df=train_df, features=features)
    X_val, X_val_id, y_val, X_val_tree, y_val_tree = make_return_sequences(
    ...
    tree_model = lgb.LGBMRegressor(
        objective='huber',
        alpha=config_manager.return_delta,
        num_leaves=128,
